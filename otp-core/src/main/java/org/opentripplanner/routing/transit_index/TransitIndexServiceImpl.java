...
public class TransitIndexServiceImpl implements TransitIndexService, Serializable {
    ...
    private Map<AgencyAndId, PreAlightEdge> preAlightEdges;

    private Map<AgencyAndId, PreBoardEdge> preBoardEdges;
    
    private Map<TripPatternKey, TableTripPattern> tableTripPatternsByTrip;

    private Map<AgencyAndId, Set<String>> directionsForRoute;

    private Map<AgencyAndId, Set<Stop>> stopsForRoute;
    ...
    private Map<AgencyAndId, Stop> stops = new HashMap<AgencyAndId, Stop>();

    private Map<AgencyAndId, Route> routes = new HashMap<AgencyAndId, Route>();
    ...
    public TransitIndexServiceImpl(HashMap<String, List<RouteVariant>> variantsByAgency,
            ...
            Map<TripPatternKey, TableTripPattern> tableTripPatternsByTrip,
            Map<AgencyAndId, Set<String>> directionsByRoute,
            Map<AgencyAndId, Set<Stop>> stopsByRoute,
            Map<AgencyAndId, Route> routes,
            Map<AgencyAndId, Stop> stops,
    ...
    public void merge(Map<String, List<RouteVariant>> variantsByAgency,
            Map<AgencyAndId, List<RouteVariant>> variantsByRoute,
            Map<AgencyAndId, RouteVariant> variantsByTrip,
            Map<AgencyAndId, PreBoardEdge> preBoardEdges,
            Map<AgencyAndId, PreAlightEdge> preAlightEdges,
    ...
    public void add(GTFSPatternHopFactory.Result result, ServiceDate serviceDate) {
        ...
        if(variant != null) {
            ...
            tableTripPatternsByTrip.put(new TripPatternKey(result.trip.getId(), serviceDate), result.tripPattern);
            return;
        }
        ...
        synchronized (this) {
            ...
            MapUtils.addToMapSet(stopsForRoute, result.trip.getRoute().getId(), result.stopPattern.getStops());
            variantsByTrip.put(result.trip.getId(), variant);
            tableTripPatternsByTrip.put(new TripPatternKey(result.trip.getId(), serviceDate), result.tripPattern);
        }
    }
    ...
    private RouteVariant createRouteVariant(GTFSPatternHopFactory.Result result) {
        ...
        log.info("Created variant for " + variant.getRoute().getId() + " with trip " + result.trip.getId());
        ...
        for(Edge e : result.edges) {
            ...
            if(e instanceof TransitBoardAlight) {
                ...
                getOrCreateSegmentForStop(ph.getBeginStop().getId(), stopSegmentMap).hopOut = ph;
                getOrCreateSegmentForStop(ph.getEndStop().getId(), stopSegmentMap).hopIn = ph;
            }
        }

        for(Stop stop : variant.getStops()) {
            RouteSegment segment = stopSegmentMap.get(stop.getId());
